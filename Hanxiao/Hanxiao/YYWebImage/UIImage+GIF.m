...
-(NSString *)sd_gitfilepathWithThis{
    //temp dictionary
    NSString *temp = NSTemporaryDirectory();
    NSString *path = [temp stringByAppendingString:@"test1.gif"];
    NSLog(@"path = %@",path);
    NSFileManager *fp = [NSFileManager defaultManager];
    if([fp fileExistsAtPath:path]){
        [fp removeItemAtPath:path error:nil];
    };
    
    //配置gif属性
    CGImageDestinationRef destion;
    CFURLRef url = CFURLCreateWithFileSystemPath(kCFAllocatorDefault, (CFStringRef)path, kCFURLPOSIXPathStyle, false);
    destion = CGImageDestinationCreateWithURL(url, kUTTypeGIF, self.images.count, NULL);
    ...
    for (UIImage *dimage in self.images) {
        NSDictionary *frameDic = [NSDictionary dictionaryWithObject:[NSMutableDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:dimage.duration],(NSString*)kCGImagePropertyGIFDelayTime, nil] forKey:(NSString*)kCGImagePropertyGIFDelayTime];
...
-(NSData *)sd_gitdataWithThis{
    NSString *path = [self sd_gitfilepathWithThis];
