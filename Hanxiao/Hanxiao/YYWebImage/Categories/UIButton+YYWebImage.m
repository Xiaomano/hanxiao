...
- (void)_yy_setImageWithURL:(NSURL *)imageURL
    ...
    NSURL *orignalURL = imageURL;
    NSURL *signaturedURL = [YYWebImageManager signaturedURL:orignalURL];
    ...
    int32_t sentinel = [setter cancelWithNewURL:signaturedURL];
    ...
    _yy_dispatch_sync_on_main_queue(^{
        ...
        if (manager.cache &&
            ...
            imageFromMemory = [manager.cache getImageForKey:[manager cacheKeyForURL:orignalURL] withType:YYImageCacheTypeMemory];
        }
        if (imageFromMemory) {
            if (!(options & YYWebImageOptionAvoidSetImage)) {
                [self setImage:imageFromMemory forState:state.integerValue];
            }
            if(completion) completion(imageFromMemory, signaturedURL, YYWebImageFromMemoryCacheFast, YYWebImageStageFinished, nil);
            return;
        }
        ...
        dispatch_async([_YYWebImageSetter setterQueue], ^{
            ...
            newSentinel = [setter setOperationWithSentinel:sentinel url:orignalURL options:options manager:manager progress:_progress transform:transform completion:_completion];
            weakSetter = setter;
        });
    });
}
...
-(void)yy_setImageWithURLString:(NSString *)imageUrlString forState:(UIControlState)state placeholder:(UIImage *)placeholder
{
    NSURL *imageURL = [NSURL URLWithString:imageUrlString];
    [self yy_setImageWithURL:imageURL forState:state placeholder:placeholder];
...
- (void)yy_setImageWithURLString:(NSString *)imageUrlString
                  forState:(UIControlState)state
               placeholder:(UIImage *)placeholder
                 transform:(YYWebImageTransformBlock)transform
                completion:(YYWebImageCompletionBlock)completion {
...
- (void)_yy_setBackgroundImageWithURL:(NSURL *)imageURL
    ...
    NSURL *originalURL = imageURL;
    NSURL *signaturedURL = [YYWebImageManager signaturedURL:originalURL];
    ...
    _yy_dispatch_sync_on_main_queue(^{
        ...
        if (manager.cache &&
            !(options & YYWebImageOptionUseNSURLCache) &&
            !(options & YYWebImageOptionRefreshImageCache)) {
            imageFromMemory = [manager.cache getImageForKey:[manager cacheKeyForURL:originalURL] withType:YYImageCacheTypeMemory];
        }
        if (imageFromMemory) {
            if (!(options & YYWebImageOptionAvoidSetImage)) {
        ...
        dispatch_async([_YYWebImageSetter setterQueue], ^{
            ...
            newSentinel = [setter setOperationWithSentinel:sentinel url:originalURL options:options manager:manager progress:_progress transform:transform completion:_completion];
            weakSetter = setter;
        });
    });
}
...
- (void)yy_setBackgroundImageWithURLString:(NSString *)imageUrlString
                            forState:(UIControlState)state
                         placeholder:(UIImage *)placeholder
                           transform:(YYWebImageTransformBlock)transform
                          completion:(YYWebImageCompletionBlock)completion{
