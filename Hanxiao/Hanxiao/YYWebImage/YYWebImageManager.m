...
- (YYWebImageOperation *)requestImageWithURL:(NSURL *)url
    ...
    // todo:设置token
    NSURL *orignalURL = url;    // 原始url
    // 签名URL
    NSURL *signaturedURL = [YYWebImageManager signaturedURL:orignalURL];
    //[NSURL URLWithString:[YYWebImageManager signaturedURL:[orignalURL absoluteString]]];

    // cache 用原始的url
    // 下载用签名的URL
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:signaturedURL];
    request.timeoutInterval = _timeout;
    request.HTTPShouldHandleCookies = (options & YYWebImageOptionHandleCookies) != 0;
    request.allHTTPHeaderFields = [self headersForURL:signaturedURL];
    ...
    YYWebImageOperation *operation = [[YYWebImageOperation alloc] initWithRequest:request
                                                                          options:options
                                                                            cache:_cache
                                                                         cacheKey:[self cacheKeyForURL:orignalURL]
                                                                         progress:progress
